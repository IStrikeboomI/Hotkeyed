...
//This is a function that wasn't in the library that I'm adding
//Takes float % MAX_INT then chops off everything after the decimal
int BigFloat::toInt() const {
	
	std::vector<int> powersOf10 = {1,10,100,1000,10000,100000,1000000,10000000,100000000,1000000000};
	//make it within MAX SIGNED INT (2^31 - 1)
	BigFloat cropped = *this % BigFloat(1 << 31 - 1);
	int val = 0;
	for (int i = 0; i < cropped.digits.size();i++) {
		val += cropped.digits[i] * powersOf10[i];
	}
	return val;
}
